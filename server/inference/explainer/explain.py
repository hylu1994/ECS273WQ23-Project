...
class Explainer:
    ...
    # GET LIST LIKE GRAPH
    def explain_graphs_return_graph_TMP(self, graph_indices):
        """
        Explain graphs.
        """
        Gs_original = []
        ...
        for graph_idx in graph_indices:
            ...
            G2_nodes = {
                list(G_orig.nodes())[i]: str(list(G_orig.nodes())[i])
                for i in range(len(list(G_orig.nodes())))
            }
            Gs_original.append([G2_nodes, G_orig.edges()])

        return Gs_original, 0
    
    # GRAPH EXPLAINER
    # get original first
    def explain_graphs_return_graph(self, graph_indices):
        """
        Explain graphs.
        """
        Gs_denoised = []
        ...
        for tmp_id in range(len(graph_indices)):
            graph_idx = graph_indices[tmp_id]
            # masked_adj = self.explain(node_idx=0, graph_idx=graph_idx, graph_mode=True)
            # G_denoised = io_utils.denoise_graph(
            #     masked_adj,
            #     0,
            ...
            #     max_component=False,
            # )
            # if G_denoised.number_of_nodes() == 0 or G_denoised.number_of_edges == 0:
            #     print(graph_idx, "no primitive generated")
            #     Gs_denoised.append(-1)
            #     Gs_original.append(-1)
            #     Nx_denoised.append(-1)
            #     Nx_original.append(-1)
            #     continue
            ...
            # Nx_denoised.append(G_denoised)

            # G1_nodes = {list(G_denoised.nodes())[i]: str(list(G_denoised.nodes())[i]) for i in range(len(list(G_denoised.nodes())))}
            # G1_edges = []
            # for (edge_s, edge_t) in G_denoised.edges():
            #     G1_edges.append((edge_s, edge_t))
            #     G1_edges.append((edge_t, edge_s))
            # G1_tmp = Graph(initialization_object=G1_edges, node_labels=G1_nodes)
            # Gs_denoised.append(G1_tmp)
            ...
            Nx_original.append(G_orig)

            G2_nodes = {list(G_orig.nodes())[i]: str(list(G_orig.nodes())[i]) for i in range(len(list(G_orig.nodes())))}
            G2_edges = []
            for (edge_s, edge_t) in G_orig.edges():
                G2_edges.append((edge_s, edge_t))
                G2_edges.append((edge_t, edge_s))
            G2_tmp = Graph(initialization_object=G2_edges, node_labels=G2_nodes)
            Gs_original.append(G2_tmp)

            # print(G1_nodes, G1_edges)
            # print(G2_nodes, G2_edges)

        return Gs_original, Gs_denoised, Nx_original, Nx_denoised
